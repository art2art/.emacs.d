...
(add-hook 'semantic-init-hooks 'art/cedet-hook)
(add-hook 'c-mode-common-hook 'art/cedet-hook)
(add-hook 'emacs-lisp-mode-hook 'art/cedet-hook)
(add-hook 'erlang-mode-hook 'art/cedet-hook)

(defun art/c-mode-cedet-hook ()
...
(add-hook 'c-mode-common-hook 'art/c-mode-cedet-hook)
...
(defun art/ede-get-local-var (fname var)
...
(global-set-key [f9] 'art/compile)

;;
(defun art/gen-std-compile-string ()
...
;;
(defun art/gen-cmake-debug-compile-string ()
  "Generates compile string for compiling CMake project in debug mode"
  (let* ((current-dir (file-name-directory
                       (or (buffer-file-name (current-buffer)) default-directory)))
         (prj (ede-current-project current-dir))
...
(defun art/gen-cmake-debug/release-compile-string ()
